...
public class BlenderBlockEntityRenderer implements BlockEntityRenderer<BlenderBlockEntity> {
    private static final ModelResourceLocation BOTTOM_MODEL = new ModelResourceLocation(
    ...
    public void render(BlenderBlockEntity blockEntity, float partialTick, PoseStack poseStack,
        ...
        // Render bottom (static part)
        BakedModel bottomModel = modelManager.getModel(BOTTOM_MODEL);
        renderModel(bottomModel, blockState, poseStack, bufferSource, packedLight, packedOverlay);

        // Render top (rotating part)
        ...
        poseStack.translate(-0.5, 0.0, -0.5);
        renderModel(topModel, blockState, poseStack, bufferSource, packedLight, packedOverlay);
        poseStack.popPose();
    }

    private void renderModel(BakedModel model, BlockState state, PoseStack poseStack,
                             MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
